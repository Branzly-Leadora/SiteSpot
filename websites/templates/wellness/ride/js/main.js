/* ============================================================
   SMASH — sdílený JS (+ animovaná čísla)
   ============================================================ */
(function () {
  'use strict';
  ...
  } else { revEls.forEach(function (el) { el.classList.add('vis'); }); }

  /* ---- animovaná čísla (count-up) ---- */
    ...
    var cio = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        ...
        function tick(now) {
          ...
          el.textContent = (dec ? val.replace('.', ',') : Math.round(val).toLocaleString('cs-CZ')) + suffix;
  ...
  if (galItems.length && lb) {
    ...
    function classFromItem(item) {
      var inner = item.querySelector('.ph'); var found = '';
    ...
    function show(i) {
      ...
      if (lbCap) lbCap.textContent = item.getAttribute('data-cap') || '';
    }
    function open(i) { show(i); lb.classList.add('open'); document.body.style.overflow = 'hidden'; }
    function close() { lb.classList.remove('open'); document.body.style.overflow = ''; }
    galItems.forEach(function (item, i) { item.addEventListener('click', function () { open(i); }); });
